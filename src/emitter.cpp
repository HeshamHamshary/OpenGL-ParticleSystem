...
Emitter::Emitter(const string& p_sFilePath){
    ...
    m_pMaterial->SetProgram("data/particle.vsh", "data/particle.fsh");
    m_pMaterial->SetBlend(true);
    m_pMaterial->SetBlendMode(wolf::BM_SrcAlpha, wolf::BM_OneMinusSrcAlpha);

    XMLDocument doc = new XMLDocument();
    if(doc.LoadFile(p_sFilePath.c_str()) != XML_SUCCESS) throw XML_ERROR_FILE_COULD_NOT_BE_OPENED;

    XMLElement* emitter = doc.FirstChildElement("emitter");
    if(emitter == nullptr) throw XML_ERROR_FILE_READ_ERROR;
    ...
    m_eType = sType == "box_emitter" ? BOX_EMITTER : 
              sType == "sphere_emitter" ? SPHERE_EMITTER:
              POINT_EMITTER;
...
void Emitter::_spawnParticle(){
    ...
    if (m_eType == POINT_EMITTER)
        ...
        float theta = Util::randNum(0.0f, 2.0f * (float)M_PI);
